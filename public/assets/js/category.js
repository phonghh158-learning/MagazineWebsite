...
fnUpdate.addEventListener('click', () => {
    ...
    document.getElementById('lbl-updated_at').style.display = 'none';
    document.getElementById('updated_at').style.display = 'none';
...
function loadIcon() {
    ...
    while (groupIcon.childElementCount > 1) {
        groupIcon.removeChild(groupIcon.lastChild);
    }

    let currentIcon = document.getElementById('icon');
    currentIcon.checked = true;
    currentIcon.disabled = false;

    for (let i = 0; i < iconClass1.length; i++) {
        let val = "<i class='bx " + iconClass1[i] + "'></i>";
        if (currentIcon.value != val) {
